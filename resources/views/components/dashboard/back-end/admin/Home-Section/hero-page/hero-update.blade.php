                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-12 p-1">
                                <img class="w-15" id="oldImgs" src="{{ asset('images/default.jpg') }}"/>
                                <br/>
                                <label class="form-label mt-2">Banner</label>
                                <input oninput="updatePreviews(this)" type="file" class="form-control" id="BannerImageUpdate">
                                <br>  
                                ...
                                <input type="text" class="form-control" id="HometitleUpdate">
...
async function updatePreviews(input, imageUrl) {
    const oldImgs = document.getElementById('oldImgs');
    
    if (input.files && input.files[0]) {
        oldImgs.src = window.URL.createObjectURL(input.files[0]);
    } else if (imageUrl) {
        oldImgs.src = imageUrl;
    } else {
        oldImgs.src = "{{ asset('images/default.jpg') }}";
    }
}
...
async function FillUpUpdateForm(id) {
    try {
        ...
        updatePreviews(document.getElementById('LogoImageUpdate'), data.logo);
